...
export default function Home() {
  ...
  useEffect(() => {
    if (!heroRef.current || !formSectionRef.current || !formCardRef.current) return;
  ...
  useEffect(() => {
    if (!stackWrapRef.current || cards.length === 0) return;
    ...
    <main>
      ...
      >
        <div className="relative w-[70%]" style={{ height: "62vh" }}>
          {cards.length === 0 ? (
            <div className="absolute inset-0 flex items-center justify-center bg-white rounded-tl-[130px] rounded-br-[130px]">
              <p className="text-gray-600">
                {loading ? "Loading cards..." : "No feature cards found."}
              </p>
            </div>
          ) : (
            cards.map((card, i) => (
              <div
                ...
                className="absolute inset-0 flex rounded-tl-[130px] rounded-br-[130px] overflow-hidden shadow-xl bg-white"
                style={{ zIndex: i }}
              >
